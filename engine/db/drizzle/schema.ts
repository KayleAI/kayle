import { pgTable, foreignKey, uuid, timestamp, pgSchema, varchar, text, uniqueIndex, index, unique, jsonb, boolean, smallint, json, bigserial, bigint, integer, inet, vector, pgEnum } from "drizzle-orm/pg-core"
...
export const contentType = pgEnum("Content Type", ['text', 'audio', 'image', 'video', 'document'])
...
export const content = pgTable("content", {
	id: uuid("id").defaultRandom().primaryKey().notNull(),
	objectId: uuid("object_id").defaultRandom(),
...
export const identitiesInAuth = auth.table("identities", {
	...
	createdAt: timestamp("created_at", { withTimezone: true, mode: 'string' }),
	updatedAt: timestamp("updated_at", { withTimezone: true, mode: 'string' }),
	email: text("email").generatedAlwaysAs(sql`lower((identity_data ->> 'email'::text))`),
	id: uuid("id").defaultRandom().primaryKey().notNull(),
},
(table) => {
	return {
...
(table) => {
	return {
		...
		uniqueVerifiedPhoneFactor: uniqueIndex("unique_verified_phone_factor").using("btree", table.userId.asc().nullsLast(), table.phone.asc().nullsLast()),
...
(table) => {
	return {
		idxModerationsVector: index("idx_moderations_vector").using("ivfflat", table.vector.asc().nullsLast().op("vector_l2_ops")),
		moderationsContentIdKey: unique("moderations_content_id_key").on(table.contentId),
